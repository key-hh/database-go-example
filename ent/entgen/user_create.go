...
func (uc *UserCreate) check() error {
	...
	if _, ok := uc.mutation.Age(); !ok {
		return &ValidationError{Name: "age", err: errors.New(`entgen: missing required field "User.age"`)}
	}
	if v, ok := uc.mutation.Age(); ok {
		if err := user.AgeValidator(v); err != nil {
			return &ValidationError{Name: "age", err: fmt.Errorf(`entgen: validator failed for field "User.age": %w`, err)}
		}
	}
	return nil
}
